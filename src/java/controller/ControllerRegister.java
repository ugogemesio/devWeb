...
public class ControllerRegister extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(senha.equals(senha2) && !dao.isCpfRegistrado(cpf) && !dao.isUsernameRegistrado(nome)){
            Usuario user = new Usuario(nome,endereco,cpf,senha,"N");
            dao.NewUser(user); 
            response.sendRedirect("Nav?page=login");
        } else {
            response.sendRedirect("Nav?page=registrar&erro=true");
        }
